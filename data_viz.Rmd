...
# Check data types
summary(nslp_eligibility_2022_data)
...
ggplot(nslp_eligibility_2022_data, aes(x = program_type, y = participation, fill = school_type)) +
  ...
  labs(title = "October 2022 National School Lunch Program Eligibility By Meal Type",
...
ggplot(nslp_eligibility_2022_data, aes(x = program_type, y = participation, fill = school_type)) +
  ...
  theme(strip.background = element_blank(),
        strip.text = element_blank()) +
  theme(plot.title = element_text(size = unit(18, "pt")))  # Increase the font size of the title to 18


#For this plot I would like to add ggplotly so I can hover over the bar charts and get the actual value for each bar chart. Then I would like to add one label for each of the four groups that says the % Free and Reduced Enrolled
...
# Make column names lowercase
tidy_cost_analysis_data_pso <- clean_names(tidy_cost_analysis_data_pso)

# Make the data frame Tidy
tidy_cost_analysis_data_pso <- pivot_longer(tidy_cost_analysis_data_pso, 
...
```{r}
ggplot(data = tidy_cost_analysis_data_pso, aes(x = program_type, y = cost, fill = budget_type)) +
  geom_bar(stat = "identity", position = "stack")+
  facet_grid(~meal_type) +
...
```{r}
tidy_cost_analysis_data_pso_grouped <- tidy_cost_analysis_data_pso %>% 
  group_by(budget_type) %>% 
  summarise(cost_per_budget_type = sum(cost))
```
